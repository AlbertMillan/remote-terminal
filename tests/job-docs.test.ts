...
describe("listJobDocs", () => {
  ...
  it("leaves gitignored scratch out", async () => {
    ...
    try {
      ...
      rmSync(join(repo, "project", "reviews"), {
        recursive: true,
        force: true,
      });
    }
...
describe("specPathOf", () => {
  const stages = (status: string, detail: string | null) => [
    { name: "design", status, detail },
  ];

  it("uses the path a design recorded, passed or parked on a question", () => {
    const job = {
      title: "A feature",
      featureId: "f-1",
      stages: stages("needs_decision", "project/a-feature.md"),
    };
    expect(specPathOf(job)).toBe("project/a-feature.md");
    expect(
      specPathOf({ ...job, stages: stages("passed", "project/a-feature.md") }),
    ).toBe("project/a-feature.md");
  });

  it("falls back to where the design stage puts a spec for a park that predates the path", () => {
    // These rows carry the old label. They are also, by definition, the jobs
    // sitting in front of the user waiting for an answer.
    const job = {
      title: "Phase 0 — bank feed spike (Enable Banking + Santander ES)",
      featureId: null,
      stages: stages("needs_decision", "Needs a decision"),
    };
    expect(specPathOf(job)).toBe(
      "project/phase-0-bank-feed-spike-enable-banking-santander.md",
    );
  });

  it("has no spec for a design that has not run", () => {
    expect(
      specPathOf({
        title: "x",
        featureId: null,
        stages: stages("running", null),
      }),
    ).toBeNull();
